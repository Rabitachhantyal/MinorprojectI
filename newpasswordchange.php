...
if (isset($_POST['change-submit'])) {
	...
	$sql = "SELECT user_password from user where user_id = '$uid'"; //read user_password if user_id from database 	and  userId from server matched

	$result = mysqli_query($conn,$sql);
	$row = mysqli_fetch_assoc($result);


	if($hash_oldpass == $row['user_password']){  //check password that entered by user and the password already present

		if($new_password == $Confirm_password){    //confirm new password
			$hash_newpass = md5($new_password);
			$query = "UPDATE user set user_password ='$hash_newpass' WHERE user_id = '$uid' "; //update password
...
<main>
	...
	<form method ="post">
		...
		<button type = "submit" name= "change-submit">submit</button>
	</form>
</main>